...
function CustomTooltip({ active, payload, label }: TooltipProps) {
  ...
  return (
    <div className="rounded-xl bg-[var(--surface)] border border-[var(--border)] shadow-hover px-3 py-2 text-xs">
      ...
      <div className="font-display tabular text-base font-semibold mt-0.5">{payload[0].value}</div>
    </div>
  );
}
    ...
    <Card className="h-full">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">How your ATS score trended across versions</CardDescription>
        </div>
      ...
      <div className="flex items-end justify-between gap-6 mb-4">
        <div>
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className="font-display tabular text-4xl font-semibold tracking-tight text-[var(--ink)]">{currentScore}</span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="score" stroke="var(--accent)" strokeWidth={2.5} fill="url(#scoreFill)"
              dot={{ r: 4, stroke: "var(--accent)", fill: "var(--surface)", strokeWidth: 2 }} activeDot={{ r: 6 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </Card>
