...
import type { Resume, Analysis, ResumeShallow, SourceType, ResumeVersion } from "@/types";

interface MakeVersionParams {
  id: string;
  label: string;
  score: number;
...
function makeVersion({ id, label, score, sourceType, createdAt }: MakeVersionParams): ResumeVersion {
  return {
    _id: id,
    label,
    sourceType,
...
function makeAnalysis({ id, versionId, atsScore, summary }: MakeAnalysisParams): Analysis {
  return {
    _id: id,
    versionId,
    atsScore,
...
export function findMockResume(id: string): Resume | undefined {
  return mockResumes.find((r) => r._id === id);
}

export function listMockResumesShallow(): ResumeShallow[] {
