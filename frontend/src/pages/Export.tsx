...
export default function Export() {
  ...
  if (error) {
    return (
      <EmptyState icon={FileText} title="Couldn't load resume" description={(error as Error).message}
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={`Export — ${resume?.title || "Resume"}`} description="Preview a clean ATS-friendly PDF and download when it looks right."
        actions={<Button variant="ghost" onClick={() => nav(`/resumes/${id}`)}><ArrowLeft size={14} /> Back to resume</Button>} />
      ...
      <Card>
        <CardHeader className="flex-wrap">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {version && (
              <PDFDownloadLink document={<ResumeDocument {...docProps} />} fileName={fileName}>
                {({ loading }) => (
                  <Button variant="accent" size="md" disabled={loading}>
                    {loading ? <><Loader2 size={14} className="animate-spin" /> Preparing…</> : <><Download size={14} /> Download PDF</>}
                  </Button>
                )}
              </PDFDownloadLink>
            )}
      ...
      {fullVersion.error && <EmptyState icon={FileText} title="Couldn't load this version" description={(fullVersion.error as Error).message} />}

      {version && (
        <Card padding="none" className="overflow-hidden">
          <PDFViewer style={{ width: "100%", height: "min(85vh, 1000px)", border: "none", borderRadius: 20 }} showToolbar={false}>
