...
function memberSince(date?: string | null): string | null {
  if (!date) return null;
  return new Date(date).toLocaleDateString(undefined, { month: "short", year: "numeric" });
}

export function ProfileCard({ user, stats }: { user?: User | null; stats: ProfileStat[] }) {
  ...
  return (
    <Card className="h-full flex flex-col items-center text-center">
      ...
      <div className="mt-3">
        <div className="font-display text-lg font-semibold tracking-tight text-[var(--ink)]">{user?.name || "Ravi Teja"}</div>
        <div className="text-xs text-[var(--ink-muted)] mt-0.5">{user?.email || "you@example.com"}</div>
        <Badge tone="accent" className="mt-2">Pro plan</Badge>
      </div>

      <div className="grid grid-cols-3 gap-3 w-full mt-5 pt-5 border-t border-[var(--border)]">
        {stats.map((s) => (
          <div key={s.label}>
            <div className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)]">{s.label}</div>
            <div className="font-display tabular text-xl font-semibold mt-0.5 text-[var(--ink)]">{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-auto pt-5 w-full space-y-3">
        <div className="grid grid-cols-2 gap-2 w-full">
          <Button variant="outline" size="sm" onClick={() => nav("/resumes")} className="w-full">
            <Upload size={13} /> Upload
          </Button>
          <Button variant="outline" size="sm" onClick={() => nav("/insights")} className="w-full">
