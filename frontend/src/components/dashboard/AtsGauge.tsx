...
type BadgeTone = "neutral" | "accent" | "warning" | "success" | "danger" | "ink";

function statusFor(score: number): { label: string; tone: BadgeTone } {
    ...
    <Card className="h-full flex flex-col">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">How well your resume parses for ATS</CardDescription>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-4 py-2">
        <div className="relative w-full max-w-[360px]">
          <svg viewBox="0 0 300 170" className="w-full h-auto block" preserveAspectRatio="xMidYMid meet">
            <defs>
            ...
            <path d={`M 20 155 A ${RADIUS} ${RADIUS} 0 0 1 280 155`} fill="none" stroke="var(--surface-2)" strokeWidth="14" strokeLinecap="round" />
            <motion.path
          ...
          <div className="absolute inset-x-0 top-[46%] flex flex-col items-center pointer-events-none">
            <div className="text-[10px] uppercase tracking-[0.16em] text-[var(--ink-muted)] font-semibold">ATS Score</div>
            <div className="font-display tabular text-[60px] font-semibold tracking-tight text-[var(--ink)] leading-none mt-1.5">{animated}</div>
            <div className="text-[11px] text-[var(--ink-muted)] mt-1">out of 100</div>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          {delta > 0 ? "+" : ""}{delta} vs last analysis
        </div>
      </div>
    </Card>
  );
