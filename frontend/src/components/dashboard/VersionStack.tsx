...
import { ArrowRight, ArrowUpRight, FileText, PenLine, TrendingUp, TrendingDown, Minus, type LucideIcon } from "lucide-react";
import { Card, CardHeader, CardTitle, CardDescription } from "@/components/ui/Card";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { cn } from "@/lib/utils";
import type { VersionStackItem } from "@/types";
...
function VersionPill({ version, delta, isLatest }: { version: VersionStackItem; delta: number | null; isLatest: boolean }) {
  const isUpload = (version.title || "").toLowerCase().includes("upload");
  const Icon = isUpload ? FileText : PenLine;
  const DeltaIcon = delta != null ? deltaIcon(delta) : null;
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        isLatest ? "bg-[var(--accent-soft)] border-[var(--accent)]/30 shadow-card" : "bg-[var(--surface-2)] border-[var(--border)]"
      )}
    >
      <div className="flex items-center justify-between mb-3">
        <div className={cn("h-7 w-7 rounded-xl flex items-center justify-center", isUpload ? "bg-[var(--surface)] text-[var(--ink-muted)]" : "bg-[var(--accent)] text-white")}>
          <Icon size={13} />
        </div>
      ...
      <div className="flex items-baseline gap-1">
        <span className="font-display tabular text-[34px] font-semibold leading-none tracking-tight">{version.score}</span>
...
function TierBar({ score }: { score: number }) {
  ...
  const ticks = [{ at: 55, label: "Fair" }, { at: 70, label: "Strong" }, { at: 85, label: "Excellent" }];
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)] font-semibold">Tier progress</div>
        <div className="text-[10px] text-[var(--ink-muted)] tabular">
          {tier.next ? `${tier.next.at - score} pts to ${tier.next.label}` : "Max tier reached"}
        </div>
      </div>
      <div className="relative h-2.5 rounded-full bg-[var(--surface-2)] overflow-hidden">
        ...
        />
        {ticks.map((t) => (
          <div key={t.at} className="absolute top-0 bottom-0 w-px bg-white/60" style={{ left: `${t.at}%` }} />
        ))}
      </div>
      <div className="relative h-4 mt-1.5 text-[9px] text-[var(--ink-muted)] tabular">
        <span className="absolute left-0">0</span>
        {ticks.map((t) => (
          <span key={t.at} className="absolute" style={{ left: `${t.at}%`, transform: "translateX(-50%)" }}>{t.at}</span>
        ))}
...
function TrajectoryChart({ versions }: { versions: VersionStackItem[] }) {
  const data = versions.map((v) => ({ label: v.label, score: v.score }));
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)] font-semibold">Score trajectory</div>
        <div className="text-[10px] text-[var(--ink-muted)]">{versions[0].label} → {versions[versions.length - 1].label}</div>
      </div>
      <div className="h-[110px] -mx-1">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 8, left: 8, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="score" stroke="var(--accent)" strokeWidth={2.5} fill="url(#vsTrajGrad)"
              dot={{ r: 4, stroke: "var(--accent)", fill: "var(--surface)", strokeWidth: 2 }} activeDot={{ r: 5 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
  ...
  return (
    <Card className="h-full flex flex-col">
      <CardHeader>
        ...
        <Button variant="ghost" size="sm" onClick={() => nav("/versions")} className="-mr-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-5 items-stretch">
        <div className="flex items-center gap-2 sm:gap-3">
          {visible.map((v, i) => {
            ...
            return (
              <Fragment key={v.id}>
                {i > 0 && <ArrowRight size={14} className="text-[var(--ink-muted)] shrink-0" />}
        ...
        <div className="flex flex-col justify-between gap-4 lg:border-l lg:border-[var(--border)] lg:pl-5">
          <div>
            <div className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)] font-semibold">Latest</div>
            <div className="font-display text-lg font-semibold tracking-tight mt-1 truncate">{resumeTitle || latest.title || latest.label}</div>
            <div className="text-xs text-[var(--ink-muted)] mt-0.5">{latest.label} · {visible.length} version{visible.length > 1 ? "s" : ""}</div>
          </div>

          <div className="space-y-2">
            <div className="text-[10px] uppercase tracking-wide text-[var(--ink-muted)] font-semibold">Since {first.label}</div>
            <div className={cn(
              ...
              {totalDelta > 0 ? "+" : ""}{totalDelta} pts overall
            </div>
          </div>

          <Button variant="accent" size="sm" disabled={!resumeId} onClick={() => resumeId && nav(`/resumes/${resumeId}`)} className="w-full">
