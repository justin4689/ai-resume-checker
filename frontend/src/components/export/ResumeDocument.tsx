...
import type { User, ResumeVersion } from "@/types";
...
export function ResumeDocument({ user, version, title }: ResumeDocumentProps) {
  const p = (version?.parsedSections as Record<string, unknown>) || {};
  const basics = (p.basics as Record<string, unknown>) || {};

  const displayName = (basics.name as string)?.trim() || user?.name || "Your Name";
  const displayEmail = (basics.email as string)?.trim() || user?.email || "";

  const experience = (p.experience as Array<{
    role?: string; period?: string; company?: string; location?: string; bullets?: string[];
  }>) || [];
  const projects = (p.projects as Array<{
    name?: string; tech?: string[]; description?: string;
    links?: Array<{ url?: string; label?: string }>;
  }>) || [];
  const education = (p.education as Array<{
    degree?: string; period?: string; school?: string; location?: string; details?: string;
  }>) || [];
  const skills = (p.skills as string[]) || [];
  const certifications = (p.certifications as Array<{
    name?: string; issuer?: string; year?: string;
  }>) || [];
  const languages = (p.languages as string[]) || [];
  const interests = (p.interests as string[]) || [];
  const links = (basics.links as Array<{ url?: string; label?: string }>) || [];
    ...
    <Document title={title || displayName || "Resume"} author={displayName}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerWrap}>
          ...
          {basics.title ? (
            <Text style={styles.title}>{basics.title as string}</Text>
          ) : null}

          <View style={styles.contactRow}>
            {basics.location ? (
              <ContactItem>{basics.location as string}</ContactItem>
            ) : null}
            {displayEmail ? (
            ...
            {basics.phone ? <ContactItem>{basics.phone as string}</ContactItem> : null}
            {links.map((l, i) => (
        ...
        {p.summary ? (
          <View style={styles.section}>
            ...
            <Text style={styles.summary}>{p.summary as string}</Text>
          </View>
        ) : null}
