...
function ProfileSection() {
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error("Couldn't update profile", (err as Error)?.message);
    } finally {
      setSaving(false);
    }
  }
    ...
    <Card padding="lg" className="max-w-2xl">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">Your display name appears on the dashboard greeting and on your resumes.</CardDescription>
        </div>
      </CardHeader>
...
interface ThemeOptionProps { value: "light" | "dark"; label: string; icon: LucideIcon; current: string; onSelect: (v: "light" | "dark") => void }

function ThemeOption({ value, label, icon: Icon, current, onSelect }: ThemeOptionProps) {
  const active = current === value;
  return (
    <button type="button" onClick={() => onSelect(value)}
      className={cn("relative flex-1 flex flex-col items-start gap-3 p-4 rounded-2xl border text-left transition-all", active ? "border-[var(--accent)] bg-[var(--accent-soft)]" : "border-[var(--border)] bg-[var(--surface)] hover:bg-[var(--surface-2)]")}>
      <div className={cn("h-9 w-9 rounded-xl flex items-center justify-center", active ? "bg-[var(--accent-strong)] text-white" : "bg-[var(--surface-2)] text-[var(--ink-muted)]")}><Icon size={16} /></div>
      <div>
        <div className="text-sm font-semibold text-[var(--ink)]">{label}</div>
        <div className="text-[11px] text-[var(--ink-muted)] mt-0.5">{value === "light" ? "Soft, airy, sage tones" : "Calm, low-glare night"}</div>
      </div>
      {active && <span className="absolute top-3 right-3 h-5 w-5 rounded-full bg-[var(--accent-strong)] text-white flex items-center justify-center"><Check size={12} /></span>}
    </button>
  );
}
    ...
    <Card padding="lg" className="max-w-2xl">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">Pick a theme. Your choice is remembered on this device.</CardDescription>
        </div>
      </CardHeader>
...
function PasswordSection() {
  ...
  const canSubmit = currentPassword.length > 0 && newPassword.length >= 8 && confirm === newPassword && !saving;
    ...
    <Card padding="lg" className="max-w-2xl">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">Use at least 8 characters. Mix letters, numbers, and a symbol for a stronger password.</CardDescription>
        </div>
      </CardHeader>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex justify-end pt-2">
          <Button type="submit" disabled={!canSubmit}>{saving ? "Updating..." : "Update password"}</Button>
        </div>
      </form>
    </Card>
  );
...
export default function Settings() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Settings" description="Manage your account, look & feel, and password." />
